const CACHE_NAME = 'harrys-chat-v34';
const STATIC_ASSETS = [
    '/',
    '/index.html',
    '/manifest.json',
...
self.addEventListener('fetch', (event) => {
    ...
    // APK 等大文件下载：绝不走 SW 缓存，避免大文件缓存截断/错误响应被缓存导致安装时 "packageInfo is null"
    if (url.pathname.endsWith('.apk') || url.pathname.endsWith('.zip') || url.pathname.endsWith('.ipa')) return;
